...
public class AdGroupCache {
	...
	private static final String STRATEGIES_AD_GROUP_ID = "strategies:adGroupId:";
	...
	private static final IMultiBidStrategyService strategyService = SpringUtil.getBean(IMultiBidStrategyService.class);

	public static List<TargetCondition> listCondition(Integer adGroupId) {
		return CacheUtil.get(AD_GROUP_CACHE,
				CONDITIONS_AD_GROUP_ID,
				adGroupId,
				() -> conditionService.listCondition(adGroupId));
	}

	public static List<MultiBidStrategy> listStrategy(Integer adGroupId) {
		return CacheUtil.get(AD_GROUP_CACHE,
				STRATEGIES_AD_GROUP_ID,
				adGroupId,
				() -> strategyService.listByAdGroupId(Collections.singletonList(adGroupId)));
	}

	public static AdGroup getAdGroup(Integer id) {
		return CacheUtil.get(AD_GROUP_CACHE, AD_GROUP_ID,
				id,
				() -> adGroupService.getById(id));
	}
