...
/**
 * 原生广告的请求，作为json字符串在{@link Native#request}中
 * 也有一部分交易所是选择将当前响应作为对象返回到{@link Native}的其他字段中
 * 标准的协议只支持JSON编码的字符串
 * <p>
 * 在1.1版本之前，当前请求是被包装多了一层map，作为native字段的值，{"native":NativeRequest},见{@link NativeRequestWrapper}
 * 再将这个map转为json字符串给{@link Native#request}
 */
@Getter
@Setter
public class NativeRequest extends Extension {
  ...
  /**
   * @see NativeLayoutIdEnum
   * @deprecated 原生广告单元的布局 ID
   */
  private Integer layout;

  /**
   * @see NativeAdUnitIdEnum
   ...
   */
  private List<NativeRequestAsset> assets;
