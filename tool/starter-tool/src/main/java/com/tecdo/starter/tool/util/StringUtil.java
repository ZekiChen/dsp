...
import com.tecdo.starter.tool.support.StrSpliter;
...
public class StringUtil extends StringUtils {
	...
	public static String format(CharSequence template, Map<?, ?> map) {
		...
		for (Map.Entry<?, ?> entry : map.entrySet()) {
			template2 = template2.replace("{" + entry.getKey() + "}", BigTool.toStr(entry.getValue()));
		}
		return template2;
	}
	...
	public static String getContainsStr(CharSequence str, CharSequence... testStrs) {
		if (isEmpty(str) || BigTool.isEmpty(testStrs)) {
			return null;
		}
		String[] strArr = split(str, ",");
		for (CharSequence checkStr : testStrs) {
			for (String s : strArr) {
				if (s.contentEquals(checkStr)) {
					return s;
				}
			}
		}
	...
	public static int count(CharSequence content, CharSequence strForSearch) {
		if (BigTool.hasEmpty(content, strForSearch) || strForSearch.length() > content.length()) {
			return 0;
		}
