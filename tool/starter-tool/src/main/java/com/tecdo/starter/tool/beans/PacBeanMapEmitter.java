 ...
 **/
class PacBeanMapEmitter extends ClassEmitter {

	private static final Type BEAN_MAP = TypeUtils.parseType(PacBeanMap.class.getName());
	...
	public PacBeanMapEmitter(ClassVisitor v, String className, Class type, int require) {
		super(v);
		...
		if (require != 0) {
			for (Iterator it = allProps.keySet().iterator(); it.hasNext(); ) {
				...
				if ((((require & PacBeanMap.REQUIRE_GETTER) != 0) && !getters.containsKey(name)) ||
					(((require & PacBeanMap.REQUIRE_SETTER) != 0) && !setters.containsKey(name))) {
					it.remove();
					getters.remove(name);
					setters.remove(name);
				}
